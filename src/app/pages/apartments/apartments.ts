...
const STATUS_LABEL: Record<string, string> = { ACTIVE: 'Activo', MAINTENANCE: 'Mantenimiento', INACTIVE: 'Inactivo' };
...
export class ApartmentsComponent implements OnInit {
  ...
  load(page = 1) {
    ...
    this.svc.findAll({ search: this.search, status: this.statusFilter, page }).subscribe({
      next: res => { this.apartments.set(res.data); this.meta.set(res.meta); this.loading.set(false); },
  ...
  openCreate() { this.newName = ''; this.showCreate.set(true); }

  submitCreate() {
    if (!this.newName.trim()) return;
    this.saving.set(true);
    this.svc.create({ internalName: this.newName.trim() }).subscribe({
      next: apt => { this.showCreate.set(false); this.saving.set(false); this.router.navigate(['/apartments', apt._id]); },
  ...
  confirmDelete() {
    ...
    this.svc.remove(id).subscribe({
      next: () => { this.showDelete.set(false); this.saving.set(false); this.load(1); },
      error: () => this.saving.set(false),
    });
  }
