...
import { ApartmentsService, IApartment, IRoom, IBathroom } from '../../../core/services/apartments.service';
...
export class ApartmentDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.svc.findById(id).subscribe({
      next: apt => { this.apt.set(this.normalize(apt)); this.syncGeneralForm(apt); this.loading.set(false); },
      error: () => { this.loading.set(false); this.router.navigate(['/apartments']); },
    });
  }
  ...
  saveGeneral() {
    ...
    this.svc.update(id, { internalName: this.editName.trim(), status: this.editStatus }).subscribe({
      next: apt => { this.apt.set(apt); this.saving.set(false); },
  ...
  saveRoom() {
    ...
    this.svc.update(apt._id, { rooms }).subscribe({
      next: updated => { this.apt.set(updated); this.showRoomForm.set(false); this.saving.set(false); },
      error: () => this.saving.set(false),
    });
  }
  ...
  saveBath() {
    ...
    this.svc.update(apt._id, { bathrooms }).subscribe({
      next: updated => { this.apt.set(updated); this.showBathForm.set(false); this.saving.set(false); },
      error: () => this.saving.set(false),
    });
  }
  ...
  saveEquipment() {
    ...
    this.svc.update(apt._id, { equipment: apt.equipment, parking: apt.parking }).subscribe({
      next: updated => { this.apt.set(updated); this.saving.set(false); },
      error: () => this.saving.set(false),
    });
  }
    ...
    this.svc.uploadImage(file).subscribe({
      next: res => {
        this.svc.addPhoto(apt._id, { url: res.url, publicId: res.publicId }).subscribe({
          next: updated => { this.apt.set(updated); this.uploadingPhoto.set(false); },
  ...
  goBack() { this.router.navigate(['/apartments']); }
}
