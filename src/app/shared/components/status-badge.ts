...
@Component({
  ...
  template: `
    <span class="px-2.5 py-1 rounded-lg text-xs font-medium inline-flex items-center gap-1.5"
          [class]="entry().cls">
      @if (showDot()) {
...
export class StatusBadge {
  ...
  protected entry() {
    return MAPS[this.type()]?.[this.value()] ?? { cls: 'bg-surface text-text-secondary border border-border', dot: 'bg-text-disabled', label: this.value() };
  }
}
